...
it will store the object that $x$ is bound to in \id.
If this setter is executed
in a situation where an object has been stored in the variable \id,
...
in a situation where no object has been stored in the variable \id{}
will set the value of \id{} to the object to which the argument $x$ is bound.
...
in a situation where no object has been stored in \id{}
...
in a situation where a value has been stored in \id{}
evaluates to the object stored in \id.

\commentary{%
Consider an instance variable declaration of the form
\code{\LATE\,\,\VAR\,\,x = $e$;}.
Perhaps surprisingly,
if a value has been stored in the variable \code{x}
...
\item \emph{Non-constant variable with an initializer.}
  ...
  in a situation where no value has been stored in the variable \id{}
  will evaluate $e$ to an object $o$, store $o$ in \id{},
  and complete returning $o$;
  if the evaluation of $e$ throws,
  the invocation of the getter throws the same object and stack trace.
  An invocation of the getter of \id{}
  in a situation where  value has been stored in the variable \id{}
  completes returning the object stored in \id.
...
\item \emph{Variable without an initializer.}
  ...
  in a situation where an object has been stored in the variable \id{}
  will complete returning that object.
  An execution of the implicitly induced getter of \id{}
  in a situation where no object has been stored in the variable \id{}
  will incur a dynamic error.
\end{itemize}
...
\item There is only one namespace
  ...
  An non-local variable $f$ introduces a getter $f$,
  and a mutable non-local variable $f$ also introduces a setter
  \code{$f$=} (\ref{instanceVariables}, \ref{variables}).
  When we speak of members here, we mean
  accessible class or instance variables,
  static or instance getters,
  static or instance setters, and
...
\item
  An expression of one of the forms \code{$e_1$\,\gtilde/\,$e_2$},
