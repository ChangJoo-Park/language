...
and it is a \Error{compile-time error} if a library variable declaration
...
where $T$ is a type
induces an implicit getter function (\ref{getters}) with header
\code{$T$\,\,\GET\,\,\id},
...
\code{\LATE?\,\,\VAR\,\,\id;} or
\code{\LATE?\,\,\FINAL\,\,\id;}
induces an implicit getter function with header
\code{\DYNAMIC\,\,\GET\,\,\id},
whose invocation evaluates as described below
(\ref{evaluationOfImplicitVariableGetters}).
...
\code{\LATE?\,\,$T$\,\,\id;} or
\code{\LATE?\,\,$T$\,\,\id{} = $e$;}
induces an implicit setter function (\ref{setters}) with header
...
induces an implicit setter function (\ref{setters}) with header
\code{\VOID\,\,\SET\,\,\id($T$\,\,$x$)},
whose first execution sets the value of \id{} to the incoming argument $x$,
...
\commentary{%
It is again assumed that type inference has taken place,
...
\commentary{%
Note that is an error for a many kinds of variable
whose type is potentially non-nullable
to have no initializing expression.%
}
...
\code{\LATE\,\,\VAR\,\,x = $e$;}.
If a value is assigned to \code{x} before the first time it is evaluated,
...
Otherwise, when the corresponding setter has not been executed without throwing,
...
\commentary{%
Note that it is a \Error{compile-time error} for an initializing formal
...
It is a
\Error{compile-time error} if a non-redirecting generative constant constructor
is declared by a class that has an instance variable with the modifiers
\LATE{} and \FINAL.
...
A \Error{compile-time error} occurs if the declared name of an extension
...
where $v$ is a fresh variable whose type is \NonNullTypeOf{$S$}:
...
It is a \Error{compile-time error} if $T$ does not have an accessible
(\ref{privacy})
...
Let $T$ be the static type of $e$.
It is a
\Error{compile-time error} if $T$ does not have a method or getter named \id{}
...
\NonNullTypeOf{$S$} and the static type of $e$.
...
\commentary{In this case, the \lit{??} is superfluous.}
The static type of $a$ is the least upper bound of
\NonNullTypeOf{$S$} and the static type of $e_2$.
...
\commentary{In this case, the \lit{??} is superfluous.}
The static type of $a$ is the least upper bound of
\NonNullTypeOf{$S$} and the static type of $e_3$.
...
It is a \Error{compile-time error}
if \NonNullTypeOf{$S$} does not have a getter named \id.
Let $U$ be the return type of said getter.
A warning occurs if $U$ is non-nullable.
\commentary{In this case, the \lit{??} is superfluous.}
The static type of $a$ is the least upper bound of
\NonNullTypeOf{$U$} and the static type of $e_2$.
...
the header of the declaration of the operator used.
...
The static type of $e$ is \NonNullTypeOf{$S$}.
...
%% TODO(eernst): Revise when flow analysis is added.
\commentary{%
If a local variable $v$ is final
...
\commentary{%
In other words, the initializing expression cannot await an expression directly,
...
It is a \Error{compile-time error} if $D$ is empty and \id{} is a final variable.
...
\LMHash{}%
We define a function \NonNullTypeOfName,
...
We cannot map it to \code{FutureOr<\NonNullTypeOf{$X$}>}
...
of \code{FutureOr<\NonNullTypeOf{$X$}>}.
...
\begin{itemize}
\item
  \DefEquals{\NonNullTypeOf{Null}}{\code{Never}}.
\item
  \DefEquals{\NonNullTypeOf{$T$?}}{\NonNullTypeOf{$T$}},
  for any type $T$.
\item
  \DefEquals{\NonNullTypeOf{$X$}}{\code{$X$\,\&\,\NonNullTypeOf{$B$}}},
  where $X$ is a type variable with bound $B$,
  such that $\NonNullTypeOf{$B$} \not= B$.
\item
  \DefEquals{\NonNullTypeOf{$X$\,\&\,$T$}}{\code{$X$\,\&\,\NonNullTypeOf{$T$}}},
  for any type $T$.
\item
  Otherwise, \DefEquals{\NonNullTypeOf{$T$}}{T},
  for any type $T$.
\end{itemize}
